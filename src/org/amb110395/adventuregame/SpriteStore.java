...
public class SpriteStore {
	...
	public Sprite[] loadSpritesFromSheet(String ref, int startRow, int endRow, int startColumn, int endColumn, int tileSize) {

		BufferedImage sheet = null;
		// if we've already got the sprite in the cache
		// then just return the existing version
		if (spriteSheets.get(ref) != null) {
			sheet = spriteSheets.get(ref);
		} else {
			// otherwise, go away and grab the sprite from the resource
			// loader
		...
		Sprite [] sprites = new Sprite[(endRow-startRow+1) * (endColumn-startColumn+1)];
		
		int i = 0;
		for (int r = startRow; r <= endRow; r++) {
			for (int c = startColumn; c <= endColumn; c++) {
				BufferedImage sourceImage = sheet.getSubimage(c*tileSize, r*tileSize, tileSize, tileSize);
				Image image = gc.createCompatibleImage(sourceImage.getWidth(),
						sourceImage.getHeight(), Transparency.BITMASK);
